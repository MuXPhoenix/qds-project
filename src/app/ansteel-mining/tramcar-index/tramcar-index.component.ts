...
export class TramcarIndexComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //       console.log(this.tc005 , this.tc00501,this.tc018);
  ...
  //   let that = this;
  //   if(ml_object_id != 0) {
  //     that.globalService.httpRequest('post', 'addMiningLog', {
  //       'ml_object_id': ml_object_id,
  //       'ml_title': msg,
  //       'ml_device_status': 5,//5：故障中
  //       'ml_classify': ml_classify,
  //       'ml_value': value,
  //       'c_id': that.cookieStore.getCookie('cid'),
  //       'ml_operator_id': that.cookieStore.getCookie('uid'),
  //       'ml_postion':that.tc001,
  //       'u_id': that.cookieStore.getCookie('uid')
  //     }).subscribe((data) => {
  //       if (data['status'] == 200) {
  //         that.mining_waring_list = data['result']['miningWaringList'];
  //         that.mining_log_list = data['result']['miningLogList'];
  //       } else if (data['status'] == 202) {
  //         alert(data['msg']);
  //         this.cookieStore.removeAll(that.rollback_url);
  //         this.router.navigate(['/auth/login']);
  //       }
  //     },
  //     response => {
